  ...
  <nav class="fixed top-0 left-0 right-0 z-50 glass border-b border-border">
    ...
    <div v-if="mobileMenuOpen" class="md:hidden glass-strong border-t border-border">
      <div class="px-4 py-3 space-y-1">
        ...
        <!-- Search + Telegram -->
        <div class="pt-3 mt-2 border-t border-border space-y-3">
          <div class="relative">
            <input
              type="text"
              v-model="searchQuery"
              placeholder="Search matches..."
              class="w-full bg-card border border-border rounded-lg pl-4 pr-9 py-2.5 text-sm text-white placeholder-text-muted focus:outline-none focus:border-primary"
              @keyup.enter="handleSearch"
            />
            <svg class="w-4 h-4 absolute right-3 top-3 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </div>
          <a
            ...
            target="_blank"
            rel="noopener"
            class="flex items-center justify-center gap-2 bg-primary hover:bg-primary-dark text-white px-4 py-2.5 rounded-lg transition-colors"
            @click="closeMobileMenu"
          >
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.446 1.394c-.14.18-.357.295-.6.295l.213-3.053 5.56-5.023c.242-.213-.054-.334-.373-.121l-6.869 4.326-2.96-.924c-.64-.203-.658-.64.135-.954l11.566-4.458c.538-.196 1.006.128.828.94z"/>
